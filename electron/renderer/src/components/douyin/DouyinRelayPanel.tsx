      ...
      if (keys.length > 10) {
        ...
        const sortedKeys = keys.sort((a, b) => {
          const aTime = localStorage.getItem(a)
            ? JSON.parse(localStorage.getItem(a) || "[]")[0]?.timestamp || 0
            : 0;
          const bTime = localStorage.getItem(b)
            ? JSON.parse(localStorage.getItem(b) || "[]")[0]?.timestamp || 0
            : 0;
          return bTime - aTime;
        });
...
const DouyinRelayPanel = ({
  ...
  const appendEvent = useCallback((entry: OtherEventEntry) => {
    ...
    setEventLog((prev: OtherEventEntry[]) => [entry, ...prev]);
  }, []);
      ...
      switch (event.type) {
        case "status": {
          ...
          if (stage === "starting") {
            ...
            const delayText = nextRetryIn
              ? ` (${Math.round(nextRetryIn)}秒后重试)`
              : "";
            let rateText = "";
            if (
            ...
            const roomId =
              (payload.room_id as string | number | undefined) ?? null;
            const attempt = payload.attempt as number | undefined;
            ...
            setStatus((prev: DouyinRelayStatus | null) =>
              prev
                ? {
                    ...prev,
        ...
        case "room_rank": {
          ...
          const ranksRaw = Array.isArray(payload.ranks)
            ...
            : [];
          const parsedRanks: RankEntry[] = ranksRaw
            .slice(0, 15)
            .map((item, index) => {
              const rank = Number((item as any).rank ?? index + 1);
              const nickname =
                ((item as any).nickname as string | undefined) || "匿名";
              const scoreStr = (item as any).score_str as string | undefined;
              const scoreNum = (item as any).score as
                | number
                | string
                | undefined;
              const avatar =
                ((item as any).avatar as string | undefined) || null;
              ...
              if (
                ...
                typeof userIdStrRaw === "string" ||
                typeof userIdStrRaw === "number"
              ) {
                userId = userIdStrRaw;
              }
  ...
  const refreshStatus = useCallback(async () => {
    try {
      const relayStatus = await getDouyinRelayStatus();
      ...
      const startDouyin = async () => {
        ...
        while (retries < maxRetries) {
          try {
            ...
            retries++;
            console.error(
              `抖音直播互动服务启动失败 (尝试 ${retries}/${maxRetries}):`,
              e,
            );
  ...
  const currentStatusText = useMemo(() => {
    if (isRunning) {
      const liveIdText =
        status?.live_id ?? (lastStartedLiveIdRef.current || "未知");
  ...
  return (
    <section className="space-y-3">
      ...
      {banner ? (
        <div
          className={`rounded-2xl border px-4 py-3 text-sm ${toneClasses[banner.tone]}`}
        >
          {banner.message}
        </div>
      ) : null}
