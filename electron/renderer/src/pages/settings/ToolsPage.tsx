    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500">
            ...
            <select
              ...
              className="timao-input text-xs py-1.5 px-3"
              value={forcedDevice}
              onChange={async (e) => {
            ...
            <button
              className="timao-outline-btn text-xs px-3 py-1.5"
        ...
        <div className="text-xs text-gray-500 space-y-1">
          ...
          {prepareLog ? (
            <pre className="mt-3 max-h-[160px] overflow-y-auto bg-gray-50 rounded-xl p-3 text-[11px] border border-gray-100">{prepareLog}</pre>
          ) : null}
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer">
              <input type="checkbox" checked={autoReload} onChange={(e) => setAutoReload(e.target.checked)} className="rounded" />
              <span>清理后自动刷新</span>
            </label>
            <button
              className="timao-outline-btn text-xs px-4 py-2"
              onClick={async () => {
                try {
            ...
            >预览</button>
            <button
              className="timao-primary-btn text-xs px-4 py-2"
              onClick={async () => {
                try {
                  setBusy(true); setMessage(null);
        ...
        {cleanPreview && (
          <div className="rounded-xl bg-gray-50 p-3 max-h-[200px] overflow-y-auto border border-gray-100">
            {cleanPreview.length === 0 ? (
              <div className="text-sm text-gray-400">暂无可清理项</div>
            ) : (
        ...
        <div className="text-xs text-gray-400 mt-3">只删除已知缓存与构建产物，不会删除源码、依赖、数据。</div>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="timao-primary-btn text-xs px-4 py-2" onClick={runSelfTest} disabled={busy}>一键自检</button>
        </div>
        <div className="grid gap-3 md:grid-cols-3 text-sm">
          <div className="rounded-xl bg-gray-50 p-4 border border-gray-100">
            <div className="text-gray-500 text-xs mb-1">后端连接</div>
            <div className={`font-semibold ${selfTest?.backendOk ? 'text-emerald-600' : 'text-gray-400'}`}>{selfTest?.backendOk ? '可用' : '未检测'}</div>
          </div>
          <div className="rounded-xl bg-gray-50 p-4 border border-gray-100">
            <div className="text-gray-500 text-xs mb-1">模型与VAD</div>
            <div className={`font-semibold ${selfTest?.modelOk ? 'text-emerald-600' : 'text-gray-400'}`}>{selfTest?.modelOk ? '就绪' : '未检测'}</div>
            <div className="text-xs text-gray-400 mt-1 truncate" title={selfTest?.details || ''}>{selfTest?.details || ''}</div>
          </div>
          <div className="rounded-xl bg-gray-50 p-4 border border-gray-100">
            <div className="text-gray-500 text-xs mb-1">WebSocket</div>
            <div className={`font-semibold ${selfTest?.wsOk ? 'text-emerald-600' : 'text-gray-400'}`}>{selfTest?.wsOk ? '可用' : '未检测'}</div>
          </div>
        </div>
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          <button className="timao-primary-btn text-sm" onClick={() => preload(['small'])} disabled={busy}>预下载轻量模型</button>
        </div>
        <div className="text-xs text-gray-400 mt-3">首次下载可能较慢，建议提前完成以免影响直播。</div>
        <div className="mt-4 rounded-xl bg-gray-50 p-4 border border-gray-100">
          <div className="text-sm font-medium text-gray-700 mb-3">缓存状态</div>
          <div className="text-xs text-gray-500 space-y-1">
            {['small'].map(sz => {
              const st = modelStatus?.cache?.[sz];
              const busy = (modelStatus?.busy || []).includes(sz);
              return (
                <div key={sz} className="flex items-center gap-4 py-2 border-b border-gray-100 last:border-b-0">
                  <span className="font-medium text-gray-700 w-16">{sz.toUpperCase()}</span>
                  <span className={busy ? 'text-amber-600' : st?.cached ? 'text-emerald-600' : 'text-gray-400'}>
                    {busy ? '下载中…' : st?.cached ? '已下载' : '未下载'}
                  </span>
                  <span className="text-gray-400">{st?.bytes ? `${(st.bytes/1024/1024).toFixed(1)} MB` : '—'}</span>
                </div>
              );
            })}
          </div>
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <label htmlFor="hotwords-file" className="text-sm text-gray-600 cursor-pointer hover:text-gray-900 transition-colors">
            导入 JSON
          </label>
          <input
            id="hotwords-file"
          ...
          />
          <button className="timao-primary-btn text-xs px-4 py-2" onClick={saveHotwords} disabled={busy}>保存</button>
          <button className="timao-outline-btn text-xs px-4 py-2" onClick={async () => {
            try {
              setBusy(true); setMessage(null);
          ...
          <button className="timao-outline-btn text-xs px-4 py-2" onClick={fetchHotwords} disabled={busy}>刷新</button>
        </div>
        <div className="text-xs text-gray-400 mb-3">可直接编辑 JSON：正确词 → 变体列表</div>
        <textarea
          id="hotwords-json-editor"
          className="timao-input w-full h-48 font-mono text-xs rounded-xl"
