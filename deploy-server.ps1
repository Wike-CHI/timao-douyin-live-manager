...
ssh "${SERVER_USER}@${SERVER_IP}" "cat > ${SERVER_PATH}/start-backend.sh << 'EOFSCRIPT'
${startScript}
EOFSCRIPT
chmod +x ${SERVER_PATH}/start-backend.sh"
...
ExecStart=${SERVER_PATH}/start-backend.sh
Restart=always
RestartSec=10
...
Write-Host "   1. 启动服务: ssh ${SERVER_USER}@${SERVER_IP} '${SERVER_PATH}/start-backend.sh'" -ForegroundColor Yellow
