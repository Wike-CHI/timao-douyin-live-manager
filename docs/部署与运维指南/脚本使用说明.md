...
### 1. `scripts/初始化与迁移/scripts/初始化与迁移/simple_setup.bat` (推荐)
...
### 2. `scripts/构建与启动/scripts/构建与启动/auto_setup_and_start_gbk.bat` (已修复)
...
### 3. `scripts/构建与启动/scripts/构建与启动/auto_setup_and_start_ps.bat` (实验性)
...
### 方法一：双击运行 (推荐)
1. 双击 `scripts/初始化与迁移/scripts/初始化与迁移/simple_setup.bat` 文件
2. 等待脚本自动完成所有安装步骤
3. 脚本会自动启动应用程序
...
# 运行脚本
scripts/初始化与迁移/scripts/初始化与迁移/simple_setup.bat
```
