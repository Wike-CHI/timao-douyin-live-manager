...
3. WebSocket连接建立成功
4. 用户打开“自动切换到直播模式”设置或在出现提示时确认切换  
   （默认开启，可在设置 > 直播体验中关闭，关闭后会弹出确认提示）
...
async function handleStart() {
  ...
  const autoSwitchEnabled = settings.autoSwitchLiveMode;
  const userConfirmed = autoSwitchEnabled
    ? true
    : await confirmSwitchDialog(); // 关闭自动切换时弹框确认
  ...
  if (audioResult.success && douyinResult.success) {
    ...
    // 若有服务失败，则进行回滚处理
    await rollbackServices({ audioResult, douyinResult });
    showError('服务未全部就绪，已恢复到全功能模式');
  }
...
**失败场景处理策略**:
- 任一服务失败时立即执行 `rollbackServices`：调用 `/stop` 端点关闭已启动的服务、释放 WebSocket 资源、重置 UI 状态。
- 记录失败原因并提供“重试”按钮；用户可在不退出全功能模式的情况下再次发起 `startLiveAudio`/`startDouyinWeb`。
- 系统层面保留最多3次自动重试（间隔2秒、指数退避），超过阈值后提示检查网络/账号信息。
...
当用户关闭“自动切换”或在直播中需要临时返回全功能界面时，可通过以下方式手动切换。
...
- **收起状态最小尺寸**: 80px × 80px（与2.2.3保持一致，只展示图标与数值）
- **展开状态最小宽度**: 280px（确保内容可读）
- **展开状态最小高度**: 180px（确保按钮可点击）
- **最大宽度**: 480px（避免遮挡过多内容）
...
async function executeStartupSequence(liveUrl: string) {
  let audioResult: ServiceResult | undefined;
  let douyinResult: ServiceResult | undefined;
  
  try {
    // 步骤1: 并行启动两个服务
    [audioResult, douyinResult] = await Promise.all([
    ...
    // 步骤5: 根据设置决定是否切换
    const autoSwitch = settings.autoSwitchLiveMode;
    const shouldSwitch = autoSwitch || await confirmSwitchDialog();
    if (shouldSwitch) {
      await delay(3000);
