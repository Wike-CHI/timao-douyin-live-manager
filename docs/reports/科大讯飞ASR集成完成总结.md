...
| 文件 | 说明 |
|------|------|
| `tests/integration/test_iflytek_asr.py` | 配置测试脚本 |
| `scripts/诊断与排障/enable_iflytek_asr.sh` | 一键启用脚本 |
| `scripts/诊断与排障/disable_iflytek_asr.sh` | 一键禁用脚本 |

### 文档

| 文件 | 说明 |
|------|------|
| `docs/runbooks/科大讯飞ASR临时方案配置指南.md` | 完整配置指南 |
| `docs/reports/科大讯飞ASR集成完成总结.md` | 本文档 |
...
```bash
./scripts/诊断与排障/enable_iflytek_asr.sh
```
...
**方法1: 一键脚本**
```bash
./scripts/诊断与排障/disable_iflytek_asr.sh
pm2 restart backend
```
...
```bash
# 测试配置
python tests/integration/test_iflytek_asr.py
...
- [配置指南](../runbooks/科大讯飞ASR临时方案配置指南.md)（`docs/runbooks/科大讯飞ASR临时方案配置指南.md`）
- [SenseVoice修复报告](../runbooks/音频转写只能识别第一句话问题修复.md)（`docs/runbooks/音频转写只能识别第一句话问题修复.md`）
...
1. 立即：测试科大讯飞配置 → `python tests/integration/test_iflytek_asr.py`
2. 短期：启用科大讯飞恢复服务 → `./scripts/诊断与排障/enable_iflytek_asr.sh`
3. 中期：修复SenseVoice问题
4. 长期：根据成本和性能选择最优方案
