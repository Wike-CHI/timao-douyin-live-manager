...
# 本地服务使用标准asyncio（移除云端async_process依赖）
...
class LiveAudioStreamService:
    ...
    async def start(self, live_url_or_id: str, session_id: Optional[str] = None) -> LiveAudioStatus:
        ...
        try:
            ...
            # 本地服务直接使用传入或生成的session_id（不依赖云端统一会话管理器）
    ...
    def _init_diarizer(self) -> None:
        ...
        try:
            ...
            # 只有当明确设置max_speakers=1时才禁用自动切换
            auto_switch = (max_speakers > 1)
            single_speaker_mode = True  # 默认启用单人模式
