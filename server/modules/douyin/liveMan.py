...
def generateSignature(wss, script_file="sign.js"):
    ...
    if not script_path.is_absolute():
        script_path = MODULE_DIR / script_path
...
class DouyinLiveWebFetcher:
    ...
    def __init__(self, live_id, abogus_file="a_bogus.js"):
        ...
        """
        abogus_path = Path(abogus_file)
        if not abogus_path.is_absolute():
            abogus_path = MODULE_DIR / abogus_path
        self.abogus_file = str(abogus_path)
        self.__ttwid = None
        self.__room_id = None
    ...
    def room_id(self):
        ...
        try:
            ...
            # 🔧 修复：与原始版本一致，打印错误但不抛异常
            print("【X】Request the live room url error: ", err)
            return None  # 返回 None，允许重试
        else:
            ...
            if not found:
                ...
                try:
                    ...
                    pass
                # 🔧 修复：不抛出异常，返回 None，让调用方处理（与原始版本行为一致）
                print("【X】No match found for roomId/room_id_str")
                return None  # 返回 None 而不是抛出异常，允许重试
    ...
    def _wsOnMessage(self, ws, message):
        ...
        :param message: 数据
        """
        # 🔧 修复：恢复原始版本的简单逻辑，不过度防御
        # 原始版本：直接解析，异常由外层 try-except 捕获
        
        try:
            # 根据proto结构体解析对象（与原始版本一致）
            ...
            # 🔧 修复：恢复原始版本的简单消息处理
            # 添加基本的 None 检查，但不过度防御
            if response.messages_list is None:
                return  # 静默处理 None，但不影响其他消息
            ...
            for msg in response.messages_list:
                ...
                try:
                    ...
                    # 🔧 修复：恢复原始版本的简单异常处理，静默处理所有解析错误
                    pass
        except Exception:
            # 🔧 修复：恢复原始版本的外层异常处理，静默处理所有异常
            # 抖音数据拉取不是100%成功，静默处理异常是正常的
            pass
