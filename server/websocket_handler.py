...
from .models import Comment, HotWord, AIScript, data_manager
from .utils.helpers import format_time, safe_get
...
class WebSocketManager:
    ...
    def start(self):
        ...
        if self.config['enable_heartbeat']:
            self.worker_thread = threading.Thread(target=self._heartbeat_loop, daemon=True)
    ...
    def _handle_connect(self, auth=None):
        ...
        try:
            ...
            # 发送欢迎消息
            self.emit_to_client(client_id, 'welcome', {
                'message': '连接成功',
                'client_id': client_id,
    ...
    def _handle_unsubscribe(self, data):
        ...
        try:
            ...
            if client_id in self.clients:
                for event_type in event_types:
                    self.clients[client_id]['subscriptions'].discard(f"event:{event_type}")
    ...
    def _heartbeat_loop(self):
        ...
        while self.is_running:
            try:
                ...
                timeout_threshold = current_time.timestamp() - self.config['auto_disconnect_timeout']
    ...
    def _disconnect_client(self, client_id: str, reason: str = 'unknown'):
        ...
        try:
            # 发送断开通知
            self.emit_to_client(client_id, 'disconnect_notice', {
                'reason': reason,
                'message': f'连接因 {reason} 被断开'
            })
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats['start_time']:
            ...
            stats['messages_per_minute'] = (stats['messages_sent'] / max(runtime.total_seconds() / 60, 1))
    ...
    def get_clients_info(self) -> List[Dict[str, Any]]:
        ...
        for client_id, client_info in self.clients.items():
            ...
            info['connected_duration'] = (datetime.now() - info['connected_at']).total_seconds()
...
class LiveDataBroadcaster:
    ...
    def broadcast_comment(self, comment: Comment):
        """广播单个评论"""
        try:
            self.ws_manager.emit_to_subscribers('comments', 'new_comment', {
    ...
    def broadcast_script(self, script: AIScript):
        """广播单个话术"""
        try:
            self.ws_manager.emit_to_subscribers('scripts', 'new_script', {
